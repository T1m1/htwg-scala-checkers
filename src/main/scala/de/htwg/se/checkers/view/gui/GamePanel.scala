...
class InjectablePanel(controllerActor: ActorRef)(implicit val bindingModule: BindingModule) extends Injectable {
  ...
  val gamePanel = new GamePanel(controllerActor, size)

  class GamePanel(controllerActor: ActorRef, size: Int) extends GridPanel(0, size + 1) {

    implicit val timeout = akka.util.Timeout(5, TimeUnit.SECONDS)

    val ALPHABET = ('A' to 'Z').toArray
    val light = Color.decode("#FCEBCC")
    val dark = Color.decode("#AA7857")
    val possible = Color.decode("#F9F89F")
    val selected = Color.decode("#B9F7BA")
    val screenSize = Toolkit.getDefaultToolkit.getScreenSize
    val dim = screenSize.height / 14
    var fields: Array[Array[Button]] = Array.ofDim[Button](size, size)
